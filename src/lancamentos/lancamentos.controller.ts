...
export const LANCAMENTO_ROUTES: BaseRoutes = {
  create: {
    ...
    route: '/',
    permission: Permission.CREATE_LANCAMENTO,
  },
  findById: {
    name: 'findById',
    route: ':id',
  ...
  update: {
    ...
    route: ':id',
    permission: Permission.UPDATE_LANCAMENTO,
  },
  delete: {
    name: 'delete Lancamento',
    route: ':id',
    permission: Permission.DELETE_LANCAMENTO,
  },
  search: {
 ...
 */
  ...
  async search(@Query() data: GetLancamentosQueryDto) {
    const { search, page, limit, status, exclude, dataInicial, dataFinal } = data;
    const response = await this.lancamentoService.findManyLocacao(search, page, limit, status, exclude, dataInicial, dataFinal);
    return response;
  }
